...
function AdminServants() {
  ...
  async function loadServants() {
    ...
    const [s, c, a] = await Promise.all([
      supabase.from('profiles')
        .select('id, first_name, last_name, active')
        .eq('role', 'servant').order('first_name'),
      supabase.from('classes')
        .select('id, name, grade_group, active')
        .eq('active', true).order('id'),
      supabase.from('servant_classes')
        .select('servant_id, class_id')
    ])

    const error = s.error || c.error || a.error
    if (error) {
      setMessage(error.message)
      setLoading(false)
      return
  ...
  function servantClasses(servantId) {
    const ids = assignments
      .filter(x => x.servant_id === servantId)
      .map(x => Number(x.class_id))
    return classes.filter(c => ids.includes(Number(c.id)))
  }

  async function toggleAssignment(servantId, classId) {
    setSaving(true)
    setMessage('')
    const assigned = assignments.some(
      x => x.servant_id === servantId &&
        Number(x.class_id) === Number(classId)
    )

    const result = assigned
      ? await supabase.from('servant_classes').delete()
          .eq('servant_id', servantId).eq('class_id', classId)
      : await supabase.from('servant_classes').insert({
          servant_id: servantId, class_id: classId
        })
    ...
    await loadServants()
    setMessage('Class assignments updated successfully.')
    setSaving(false)
  }

  const visible = servants.filter(servant => {
    const name =
      `${servant.first_name || ''} ${servant.last_name || ''}`.toLowerCase()
    const assigned = servantClasses(servant.id)
    return name.includes(search.toLowerCase()) &&
      (classFilter === 'all' ||
        assigned.some(c => String(c.id) === classFilter))
  })

  const activeCount = servants.filter(x => x.active).length
  const unassignedCount = servants.filter(
    x => servantClasses(x.id).length === 0
  ).length

  if (selectedServant) {
    const assigned = servantClasses(selectedServant.id)
      ...
      <>
        <button
          ...
          style={{
            ...
            padding: 0, marginBottom: '18px', cursor: 'pointer',
            color: '#6b35c0', fontWeight: '700'
          }}
        >
          <ArrowLeft size={18} /> Back to Servants
        </button>

        <DashboardHeader
          title={`${selectedServant.first_name} ${selectedServant.last_name}`}
          subtitle="Servant profile and class assignments"
        />
        ...
        <section className="dashboard-card">
          ...
          <p style={{ color: '#6b7280', marginTop: '-8px' }}>
            Check every class this servant should be able to manage.
          </p>
          ...
          <div style={{
            ...
            {classes.map(classItem => {
              const checked = assigned.some(
                x => Number(x.id) === Number(classItem.id)
              )
              return (
                <label key={classItem.id} style={{
                  ...
                  border: checked
                    ? '1px solid #bda5ee'
                    : '1px solid #e6e7ec',
                  background: checked ? '#f7f3ff' : 'white',
                  ...
                  <input
                    ...
                    disabled={saving}
                    onChange={() =>
                      toggleAssignment(selectedServant.id, classItem.id)
                    }
                  />
                  <div>
                    ...
                    <span style={{ color: '#6b7280', fontSize: '12px' }}>
                      {classItem.grade_group || 'Bible Study class'}
                    </span>
                  </div>
                </label>
              )
            ...
            <table>
              ...
              <tbody>
                ...
                {!assigned.length && (
                  <tr>
                    <td colSpan="3">
                      This servant is not assigned to a class yet.
                    </td>
                  </tr>
                )}
              </tbody>
  ...
  return (
    <>
      <DashboardHeader
        title="Servants"
        subtitle="View servants and manage class assignments"
      />
      ...
      <div className="stats-grid">
        ...
        <StatCard
          ...
          label="Classes"
          value={classes.length}
          helper="Active Bible Study groups"
        />
        <StatCard
          icon={<Users />}
          label="Unassigned"
          value={unassignedCount}
          helper="Need a class assignment"
        />
      </div>

      {message && !selectedServant && (
        <section className="dashboard-card"><p>{message}</p></section>
      )}
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              borderRadius: '12px', border: '1px solid #dfe2ea',
      ...
      <section className="dashboard-card">
        <div style={{
          ...
          <h2 style={{ margin: 0 }}>Servant Directory</h2>
          <span style={{ color: '#6b7280', fontSize: '13px' }}>
            {visible.length} {visible.length === 1 ? 'servant' : 'servants'}
          </span>
        </div>
          ...
          <div className="table-wrapper">
            <table>
              ...
              <tbody>
                {visible.map(servant => {
                  ...
                  return (
                    <tr key={servant.id}>
                      ...
                      <td>{servant.active ? 'Active' : 'Inactive'}</td>
                      <td style={{ textAlign: 'right' }}>
                        <button
                          type="button"
                ...
                {!visible.length && (
                  <tr>
                    <td colSpan="4">No servants match these filters.</td>
                  </tr>
                )}
              </tbody>
            </table>
...
function AdminReports() {
  ...
  async function loadReports() {
    ...
    const [
      ...
      supabase
        .from('homework_quizzes')
        .select('id, class_id, title, bible_study_date, due_date')
        .order('bible_study_date', { ascending: false })
    ])
  ...
  const selectedStudentIds = selectedClassId
    ? memberships
        .filter(
          (item) => Number(item.class_id) === selectedClassId
        )
  ...
  const filterByClassOrStudent = (records) =>
    records.filter((record) => {
      if (!selectedClassId) {
        return selectedIdSet.has(record.student_id)
      }

      if ('class_id' in record && record.class_id != null) {
        return (
          Number(record.class_id) === selectedClassId &&
          selectedIdSet.has(record.student_id)
        )
      }
  ...
  const filteredAttendance = filterByClassOrStudent(attendance)
  const filteredReading = filterByClassOrStudent(reading)
  const filteredHomework = filterByClassOrStudent(homework)
  const filteredVerses = filterByClassOrStudent(verses)
  const filteredBibles = filterByClassOrStudent(bibles)
  const filteredParticipation = filterByClassOrStudent(participation)
  const filteredBonus = filterByClassOrStudent(bonus)
  ...
  const attendanceDone = countTrue(
    filteredAttendance,
    'present'
  )
  const readingDone = countTrue(
    filteredReading,
    'completed'
  )
  const homeworkDone = countTrue(
    filteredHomework,
    'completed'
  )
  const verseDone = countTrue(
    filteredVerses,
    'completed'
  )
  const bibleDone = countTrue(
    filteredBibles,
    'brought_bible'
  )
  ...
  const readingPercent = percent(
    readingDone,
    filteredReading.length
  )
  const homeworkPercent = percent(
    homeworkDone,
    filteredHomework.length
  )
  const versePercent = percent(
    verseDone,
    filteredVerses.length
  )
  const biblePercent = percent(
    bibleDone,
    filteredBibles.length
  )

  const participationPoints = filteredParticipation.reduce(
    (sum, row) => sum + (Number(row.points) || 0),
    0
  )

  const bonusPoints = filteredBonus.reduce(
    (sum, row) => sum + (Number(row.points) || 0),
    0
  )
  ...
  const filteredQuizzes = quizzes.filter((quiz) =>
    selectedClassId
      ? Number(quiz.class_id) === selectedClassId
      : true
  )

  const filteredQuizIds = new Set(
    filteredQuizzes.map((quiz) => Number(quiz.id))
  )

  const filteredSubmissions = submissions.filter(
    (submission) =>
      filteredQuizIds.has(Number(submission.quiz_id)) &&
      selectedIdSet.has(submission.student_id)
  )

  const quizAverage = filteredSubmissions.length
    ? Math.round(
  ...
  const classRows = classes.map((classItem) => {
    const classStudentIds = new Set(
      memberships
        .filter(
          (item) =>
            Number(item.class_id) === Number(classItem.id)
        )
        .map((item) => item.student_id)
    )
    ...
    const classAttendance = attendance.filter(
      (record) =>
        Number(record.class_id) === Number(classItem.id) &&
    ...
    const classReading = reading.filter((record) =>
      classStudentIds.has(record.student_id)
    )
    ...
    return {
      ...
      attendance: percent(
        countTrue(classAttendance, 'present'),
        classAttendance.length
      ),
      reading: percent(
        countTrue(classReading, 'completed'),
        classReading.length
      ),
      homework: percent(
        countTrue(classHomework, 'completed'),
        classHomework.length
      ),
      verse: percent(
        countTrue(classVerses, 'completed'),
        classVerses.length
      ),
      bible: percent(
        countTrue(classBibles, 'brought_bible'),
        classBibles.length
      )
    }
  })
  ...
  const studentRows = filteredStudents.map((student) => {
    ...
    const studentAttendance = own(filteredAttendance)
    const studentReading = own(filteredReading)
    const studentHomework = own(filteredHomework)
    const studentVerses = own(filteredVerses)
    const studentBibles = own(filteredBibles)
    const studentParticipation = own(filteredParticipation)
    const studentBonus = own(filteredBonus)
    ...
    const ownSubmissions = filteredSubmissions.filter(
      (submission) =>
        submission.student_id === student.id
    )

    const ownQuizAverage = ownSubmissions.length
      ? Math.round(
          ownSubmissions.reduce(
            (sum, submission) =>
              sum + (Number(submission.percentage) || 0),
            0
  ...
  const lowProgressStudents = studentRows
    .map((student) => ({
      ...student,
      overall: Math.round(
        (
  ...
  const reportTitle =
    classFilter === 'all'
      ...
      : classes.find(
          (classItem) =>
            String(classItem.id) === classFilter
        )?.name || 'Selected Class'
  ...
  return (
    <>
      <DashboardHeader
        title="Reports"
        subtitle="Academy-wide analytics and class comparison"
      />
      ...
      <section className="dashboard-card">
        ...
        >
          <div>
            ...
            >
              Viewing: <strong>{reportTitle}</strong>
            </p>
          </div>
        ...
        <>
          <div className="stats-grid">
            ...
            <StatCard
              ...
              helper={`${filteredSubmissions.length} submissions`}
            />
          </div>
          ...
          <section className="dashboard-card">
            ...
            {!lowProgressStudents.length ? (
              ...
              >
                ✓ No students are currently below 60% overall progress.
              </div>
            ) : (
              <div className="table-wrapper">
                <table>
